...
describe('setup-node', () => {
  ...
  describe('manifest retry and validation', () => {
    ...
    it('fails after the maximum number of manifest fetch attempts', async () => {
      let calls = 0;
      getManifestSpy.mockImplementation(() => {
        calls++;
      ...
      expect(calls).toBe(3);
      expect(cnSpy).toHaveBeenCalledWith(
        `::error::Failed to fetch a valid manifest after 3 attempts. Last error: Error${osm.EOL}`
      );
    }, 10000);
  ...
  describe('node version verification', () => {
    ...
    it('passes when the installed node version matches the expected version', async () => {
      const toolPath = path.normalize('/cache/node/12.16.1/x64');
      findSpy.mockReturnValue(toolPath);
      getExecOutputSpy.mockImplementation(async () => ({
        stdout: 'v12.16.1\n',
      ...
      expect(logSpy).toHaveBeenCalledWith(`Found in cache @ ${toolPath}`);
      expect(cnSpy).not.toHaveBeenCalledWith(
